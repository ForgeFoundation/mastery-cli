...
class ExtensionManager {
        ...
        try {
            ...
            for (const item of items) {
                if (item.isDirectory() && !item.name.startsWith('.')) {
                    const extensionPath = path.join(this.extensionsDir, item.name);
    ...
    loadExtension(extensionInfo) {
        try {
            ...
            if (typeof ExtensionClass !== 'function') {
                throw new Error(`Extension ${extensionInfo.name} does not export a class`);
            }
            ...
            if (!(extensionInstance instanceof ExtensionModel)) {
                throw new Error(`Extension ${extensionInfo.name} must extend ExtensionModel`);
            }
            
            // Validate required methods
            if (typeof extensionInstance.getHandles !== 'function') {
                throw new Error(`Extension ${extensionInfo.name} must implement getHandles() method`);
            }
            
            extensionInstance._extensionInfo = extensionInfo;
            
            this.logger.info(`✓ Loaded extension: ${extensionInstance.name} v${extensionInstance.version}`);
            
            return extensionInstance;
            
        } catch (error) {
            this.logger.error(`✗ Failed to load extension ${extensionInfo.name}:`, error.message);
            return null;
        }
    }
    ...
    registerExtension(extension, context = {}) {
        try {
            ...
            if (!handles || typeof handles !== 'object') {
                throw new Error(`Extension ${extension.name} getHandles() must return an object`);
            }
            
            for (const [command, handler] of Object.entries(handles)) {
                if (typeof handler !== 'function') {
                    this.logger.warn(`Extension ${extension.name}: Handler for '${command}' is not a function`);
                    continue;
                }
                
                // Check for command conflicts
                if (this.commandHandlers.has(command)) {
                    const existingExtension = this.commandHandlers.get(command).extensionName;
                    this.logger.warn(`Command conflict: '${command}' from ${extension.name} conflicts with ${existingExtension}`);
                    continue;
                }
            ...
            this.logger.error(`✗ Failed to register extension ${extension.name}:`, error.message);
        }
    }
    ...
    loadAllExtensions(context = {}) {
        ...
        this.logger.info(`Discovered ${discoveredExtensions.length} extension(s)`);
        ...
        this.logger.info(`Successfully loaded ${loadedCount}/${discoveredExtensions.length} extension(s)`);
    ...
    async executeHooks(hookName, data = null) {
        ...
        for (const { handler, extensionName } of hookHandlers) {
            try {
                ...
                this.logger.error(`Hook execution failed for ${extensionName}:${hookName}:`, error.message);
    ...
    unloadExtension(extensionName) {
        try {
            ...
            for (const [hookName, hookHandlers] of this.hooks.entries()) {
                const filtered = hookHandlers.filter(h => h.extensionName !== extensionName);
            ...
            this.logger.error(`✗ Failed to unload extension ${extensionName}:`, error.message);
            return false;
        }
    }
